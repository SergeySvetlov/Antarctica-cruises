...
const editTextMobile = () => {
  if (windowSize <= '767') {
    ...
    if (headerBottom !== null) {
      headerBottom.innerHTML = 'Круизы в Антарктику';
    }
    if (paragraphAbout !== null) {
    ...
    if (feedbackText !== null) {
      feedbackText.innerHTML = 'Забронируйте тур за 3-6 месяцев<br> до даты выезда и сэкономьте до 50%';
    }
    if (paragraphForm !== null) {
      paragraphForm.innerHTML = 'Оставьте заявку и мы свяжемся с вами в ближайшее время';
    }
    if (paragraphDescription !== null) {
      paragraphDescription.innerHTML = 'У вас ещё остались вопросы или сомнения? Приезжайте к нам в офис и мы вам все расскажем.';
    }
